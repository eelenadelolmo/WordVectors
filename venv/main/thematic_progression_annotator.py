        ...
        for sentence in tree.iter('sentence'):
            ...
            for child in sentence:
                ...
                if child.tag == 'rheme':
                    ...
                    for e in rheme_id_all:
                        if e[0] in rheme_xml.strip() and e[1] not in id_added_rheme:
                            regex = r'</token><token pos="[^>]+?">'.join(e[0].strip().replace("(", "\(").replace(")", "\)").split())
        ...
        for sentence_str in sentences_str:
            ...
            for tup in to_ann_list:
                ...
                if lista_rangos.count(tup[0]) == 1:
                    ...
                    if prin >= fin_old and '<m' not in rheme_str[prin:fin] and '</m' not in rheme_str[prin:fin] and rheme_str[fin-1] != '<':
